...
class TributeService:
    ...
    async def process_webhook(self, payload_or_data) -> dict[str, Any] | None:
        try:
            ...
            # NEVER fall back to a forgeable `tribute_<tg>_<amount>` payment_id —
            # downstream code (webhook handler) treats payment_id as the
            # idempotency key. A forgeable id allows arbitrary balance top-ups
            # if a webhook signing key is leaked.
            if not payment_id:
                logger.error(
                    '❌ Tribute webhook без provider-issued payment_id — отклоняем',
                    telegram_user_id=telegram_user_id,
                )
                return None
