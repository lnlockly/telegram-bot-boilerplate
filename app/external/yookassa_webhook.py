...
# Verification outcome codes for YooKassa webhook HMAC signatures.
YOOKASSA_SIG_OK = 'ok'  # signature verified OR dev-mode skip allowed
YOOKASSA_SIG_REFUSE_MISCONFIG = 'refuse_misconfig'  # secret missing in prod → 503
YOOKASSA_SIG_INVALID = 'invalid'  # header missing/malformed/mismatch → 401


def verify_yookassa_signature(body: bytes, signature_header: str | None) -> str:
    """Verify YooKassa Webhook v2 HMAC-SHA256 signature.

    YooKassa signs the raw request body with the merchant-cabinet secret and
    sends ``v1=<hex>`` in the ``Yookassa-Signature`` header (also accepted as
    ``Signature`` for compatibility with the receiver's existing header
    aliases). Returns one of the YOOKASSA_SIG_* constants. NEVER logs or
    returns the signature value itself.
    """
    secret = (getattr(settings, 'YOOKASSA_WEBHOOK_SECRET', None) or '').strip()

    if not secret:
        if getattr(settings, 'DEBUG', False):
            logger.warning(
                '⚠️ YOOKASSA_WEBHOOK_SECRET не настроен (DEBUG=True): пропускаем проверку подписи. '
                'НЕ ОСТАВЛЯЙТЕ ТАК В ПРОДАКШЕНЕ.',
            )
            return YOOKASSA_SIG_OK
        logger.error(
            '🚫 YOOKASSA_WEBHOOK_SECRET не настроен — отклоняем webhook '
            '(защита от подделки top-up при misconfig прокси)',
        )
        return YOOKASSA_SIG_REFUSE_MISCONFIG

    if not signature_header:
        logger.warning(
            'YooKassa webhook: подпись отсутствует',
            signature_present=False,
            valid=False,
        )
        return YOOKASSA_SIG_INVALID

    header = signature_header.strip()
    received_hex: str | None = None
    if header.lower().startswith('v1='):
        received_hex = header[3:].strip()
    elif header and all(ch in '0123456789abcdefABCDEF' for ch in header):
        # Some integrations forward the bare hex (no v1= prefix).
        received_hex = header

    if not received_hex:
        logger.warning(
            'YooKassa webhook: некорректный формат подписи',
    ...
    if not hmac.compare_digest(received_hex.lower(), expected_hex):
        logger.warning(
            'YooKassa webhook: подпись не прошла проверку',
            signature_present=True,
            valid=False,
        )
        return YOOKASSA_SIG_INVALID

    logger.info('✅ YooKassa webhook подпись подтверждена', signature_present=True, valid=True)
    return YOOKASSA_SIG_OK
...
class YooKassaWebhookHandler:
    ...
    async def handle_webhook(self, request: web.Request) -> web.Response:
        try:
            ...
            logger.debug('📄 Body received', length=len(body_bytes))

            signature = (
                request.headers.get('Yookassa-Signature')
                or request.headers.get('Signature')
                or request.headers.get('X-YooKassa-Signature')
            )
            verification = verify_yookassa_signature(body_bytes, signature)
            if verification == YOOKASSA_SIG_REFUSE_MISCONFIG:
                return web.Response(status=503, text='Webhook signature not configured')
            if verification == YOOKASSA_SIG_INVALID:
