...
def create_payment_router(bot: Bot, payment_service: PaymentService) -> APIRouter | None:
    ...
    if settings.is_yookassa_enabled():
        ...
        async def yookassa_webhook(request: Request) -> JSONResponse:
            ...
            verification = yookassa_webhook_module.verify_yookassa_signature(body_bytes, signature)
            if verification == yookassa_webhook_module.YOOKASSA_SIG_REFUSE_MISCONFIG:
                return JSONResponse(
                    {'status': 'error', 'reason': 'webhook_not_configured'},
                    status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
                )
            if verification == yookassa_webhook_module.YOOKASSA_SIG_INVALID:
                return JSONResponse(
                    {'status': 'error', 'reason': 'invalid_signature'},
                    status_code=status.HTTP_401_UNAUTHORIZED,
                )
