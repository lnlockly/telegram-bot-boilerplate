...
_FAKE_BOT_TOKEN_RE = re.compile(r'^0+:fake.*$')
...
class Settings(BaseSettings):
    ...
    def _validate_bot_token(cls, value: object) -> str:
        ...
        if not token:
            raise ValueError('BOT_TOKEN must not be empty')
        # Allow the special fake token used by tests/CI smoke checks
        # (deterministic pattern, never a real Telegram bot token).
        if _FAKE_BOT_TOKEN_RE.match(token):
            return token
        if not _BOT_TOKEN_RE.match(token):
            raise ValueError(
                'BOT_TOKEN must match Telegram format <bot_id>:<hash>; '
                f'got value of length {len(token)}'
            )
        return token
    ...
    # Canonical Tribute checkout URL (see https://tribute.tg docs). When unset,
    # TributePaymentMixin.create_tribute_payment raises NotImplementedError
    # instead of returning a forged placeholder link.
    TRIBUTE_DONATIONS_URL: str | None = None
    TRIBUTE_WEBHOOK_PATH: str = '/tribute-webhook'
    TRIBUTE_WEBHOOK_HOST: str = '0.0.0.0'
    TRIBUTE_WEBHOOK_PORT: int = 8081

    # --- Webhook restore-missing-payment guards (HIGH fix #9) ----------------
    # When False (default), CloudPayments/YooKassa webhooks for unknown invoice
    # ids are acknowledged with 200 OK and a warning log instead of creating a
    # new local payment row from webhook-supplied amount/user_id. Combined with
    # a leaked signing key, the legacy path would let an attacker forge any
    # amount to any user.
    ALLOW_RESTORE_MISSING_PAYMENTS: bool = False
    # Hard cap (in kopeks) when restore IS enabled. Anything above goes to
    # manual review (error log) and is NOT credited. Default ₽5000.
    RESTORE_MISSING_PAYMENT_MAX_KOPEKS: int = 500_000

    # Optional Fixer.io API key. Without it, the fixer source is skipped
    # entirely (no broken URL with literal `YOUR_API_KEY`).
    ...
    # HMAC-SHA256 secret for YooKassa Webhook v2 signature verification.
    # Configured in YooKassa merchant cabinet. When empty in production the
    # webhook receiver refuses requests (defence against misconfig that
    # would otherwise allow forged top-ups behind an IP allowlist).
    ...
    def get_cabinet_jwt_secret(self) -> str:
        ...
        if not self.DEBUG:
            raise RuntimeError(
                'CABINET_JWT_SECRET is required in production. '
                'Set it to a random 32+ byte secret: '
                'python -c "import secrets; print(secrets.token_urlsafe(64))"'
            )
        logger.warning(
            'CABINET_JWT_SECRET is not set, falling back to BOT_TOKEN (DEBUG mode only). '
