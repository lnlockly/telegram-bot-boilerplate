...
class BanNotificationService:
    ...
    async def _find_user_by_identifier(self, db: AsyncSession, user_identifier: str) -> User | None:
        ...
        """
        # Subscription/Remnawave-based lookup removed in boilerplate.
        # TODO(boilerplate): if you have an external identifier mapping,
        # plug your lookup here. By default, only direct email match is used.
        try:
            result = await db.execute(select(User).where(User.email == user_identifier).limit(1))
            user = result.scalar_one_or_none()
            if user:
                return user
        except Exception as e:
            logger.warning('Ошибка поиска пользователя по email', error=e)
        return None
