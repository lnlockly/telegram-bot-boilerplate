...
from app.database.models import BroadcastHistory, User, UserStatus
...
async def cleanup_blocked_broadcast_users(blocked_telegram_ids: list[int]) -> None:
    ...
    Помечает пользователя как BLOCKED. (Subscription/Remnawave-cleanup removed
    in the boilerplate; plug your own post-block cleanup if needed.)
    """
...
class EmailBroadcastService:
    ...
    async def _fetch_email_recipients(self, target: str) -> list[_EmailRecipient]:
        ...
        async with AsyncSessionLocal() as session:
            ...
            elif target in ('active_email', 'expired_email'):
                # Subscription-based targets removed in boilerplate.
                # TODO(boilerplate): replace with your own activity filter.
                logger.warning(
                    'Subscription-based email target requested but subscriptions are disabled in boilerplate',
                    target=target,
                )
                return []
