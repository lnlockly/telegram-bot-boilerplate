...
class YooKassaPaymentMixin:
    ...
    async def _process_successful_yookassa_payment(
        ...
        try:
            ...
            if user:
                # Subscription/trial activation paths removed in boilerplate.
                # All YooKassa payments now credit the user's balance.
                # TODO(boilerplate): plug your post-payment business logic here.
                if False:  # legacy trial / simple_subscription branches disabled
                    pass
                else:
                    ...
                    # Subscription preloading removed in boilerplate.
                ...
                # Subscription activation removed in boilerplate.
    ...
    async def process_yookassa_webhook(
        ...
        # Webhook replay protection (HIGH #8). YooKassa v2 doesn't supply a
        # per-event uuid in the body, so we synthesize a stable id from event
        # type + object id + status. Identical re-delivery of the same status
        # transition is treated as duplicate; status updates (waiting → succeeded)
        # still pass through because the composite key differs.
        from app.utils.webhook_replay import claim_webhook_event

        event_type = event.get('event') or 'unknown'
        replay_event_id = f'{event_type}:{yookassa_payment_id}:{event_object.get("status", "")}'
        if not await claim_webhook_event(db, 'yookassa', replay_event_id):
            logger.info(
                'YooKassa webhook повтор — игнорируем',
    ...
    async def _restore_missing_yookassa_payment(
        ...
        # HIGH #9: Refuse to materialise a payment from webhook body unless the
        # operator has explicitly opted in. With a leaked signing key this path
        # let an attacker forge any amount to any user.
        if not getattr(settings, 'ALLOW_RESTORE_MISSING_PAYMENTS', False):
            logger.warning(
                'YooKassa: пропускаем восстановление платежа (ALLOW_RESTORE_MISSING_PAYMENTS=false)',
                yookassa_payment_id=yookassa_payment_id,
            )
            return None

        # Cap restored amounts. Anything above goes to manual review.
        try:
            _amount_value = (event_object.get('amount') or {}).get('value')
            _restore_amount_kopeks = (
                int((Decimal(str(_amount_value)) * 100).quantize(Decimal(1)))
                if _amount_value is not None
                else None
            )
        except (InvalidOperation, ValueError, TypeError):
            _restore_amount_kopeks = None
        max_kopeks = getattr(settings, 'RESTORE_MISSING_PAYMENT_MAX_KOPEKS', 500_000)
        if _restore_amount_kopeks is not None and _restore_amount_kopeks > max_kopeks:
            logger.error(
                'YooKassa restore exceeds RESTORE_MISSING_PAYMENT_MAX_KOPEKS — '
                'manual review required',
                yookassa_payment_id=yookassa_payment_id,
                amount_kopeks=_restore_amount_kopeks,
                max_kopeks=max_kopeks,
            )
            return None
