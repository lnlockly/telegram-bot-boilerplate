...
# Subscription renewal logic removed in boilerplate.
# TODO(boilerplate): plug your post-payment business logic here.
RenewalPaymentDescriptor = Any  # type: ignore[misc, assignment]


def decode_payment_payload(*_args, **_kwargs):  # type: ignore[no-redef]
    return None


def parse_payment_metadata(*_args, **_kwargs):  # type: ignore[no-redef]
    return None
...
class CryptoBotPaymentMixin:
    ...
    async def create_cryptobot_payment(
        ...
        try:
            # Avoid float-formatting drift: route through Decimal for the
            # two-decimal string the API consumes and the payload-derived
            # cents value.
            try:
                amount_decimal = Decimal(str(amount_usd))
            except (InvalidOperation, ValueError, TypeError):
                logger.error('Некорректная сумма USD для CryptoBot invoice', amount_usd=amount_usd)
                return None
            amount_str = format(amount_decimal.quantize(Decimal('0.01'), rounding=ROUND_HALF_EVEN), 'f')
            amount_cents = int(
                (amount_decimal * 100).quantize(Decimal('1'), rounding=ROUND_HALF_EVEN)
            )
            ...
            invoice_data = await self.cryptobot_service.create_invoice(
                ...
                payload=payload or f'balance_topup_{user_id}_{amount_cents}',
    ...
    async def process_cryptobot_webhook(
        ...
        try:
            ...
            # Webhook replay protection (HIGH #8). CryptoBot's outer envelope
            # carries a per-delivery update_id; fall back to invoice_id when a
            # synthetic webhook is built from the polling fallback path.
            from app.utils.webhook_replay import claim_webhook_event

            update_id = webhook_data.get('update_id') or invoice_id
            if not await claim_webhook_event(db, 'cryptobot', f'invoice_paid:{update_id}'):
                logger.info(
                    'CryptoBot webhook повтор — игнорируем',
            ...
            if not updated_payment.transaction_id:
                # Use Decimal end-to-end for USD/RUB math. amount_float on the
                # model is a float-coercion of the original Decimal/str, so we
                # re-anchor on the raw `amount` string to avoid float drift.
                try:
                    amount_usd = Decimal(str(updated_payment.amount))
                except (InvalidOperation, ValueError, TypeError):
                    logger.error(
                        'CryptoBot: некорректное значение amount у платежа',
                        invoice_id=invoice_id,
                        amount=getattr(updated_payment, 'amount', None),
                    )
                    return False
                ...
                try:
                    ...
                    logger.error(
                        'CryptoBot: rate fetch raised, payment requires manual review',
                        invoice_id=invoice_id,
                        error=error,
                    )
                    return False
                ...
                if amount_rubles is None:
                    # No 1:1 or hardcoded fallback — refuse to credit and queue
                    # for manual review. Crediting at any guessed rate
                    # under-/over-pays by orders of magnitude.
                    logger.error(
                        'CryptoBot rate unavailable, payment requires manual review',
                        invoice_id=invoice_id,
                        amount_usd=str(amount_usd),
                    )
                    return False

                # ceil to the next whole rouble, then to kopeks via Decimal.
                amount_rubles_rounded = int(
                    amount_rubles.quantize(Decimal('1'), rounding=ROUND_CEILING)
                )
                amount_kopeks = int(
                    (Decimal(amount_rubles_rounded) * 100).quantize(
                        Decimal('1'), rounding=ROUND_HALF_EVEN
                    )
                )
                conversion_rate = (
                    float(amount_rubles / amount_usd) if amount_usd > 0 else 0.0
                )
                logger.info(
                    'Конвертация USD->RUB',
                    amount_usd=str(amount_usd),
                    amount_rubles=str(amount_rubles),
    ...
    async def _process_subscription_renewal_payment(
        ...
        # Subscription renewal removed in boilerplate.
        # TODO(boilerplate): plug your renewal logic here. Returning False so the
        # caller falls through to the standard balance-topup branch.
        return False
