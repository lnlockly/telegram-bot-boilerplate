...
class CloudPaymentsPaymentMixin:
    ...
    async def process_cloudpayments_pay_webhook(
        ...
        # Decimal-safe rubles→kopeks conversion. `int(round(amount * 100))` on a
        # JSON float can flip the last kopek; mirror the pattern used in
        # yookassa.py:1251.
        try:
            amount_kopeks = int(
                (Decimal(str(amount)) * 100).to_integral_value(rounding=ROUND_HALF_EVEN)
            )
        except (InvalidOperation, ValueError, TypeError):
            logger.error('CloudPayments webhook: некорректная сумма', amount=amount, invoice_id=invoice_id)
            return False
        account_id = webhook_data.get('account_id', '')
        token = webhook_data.get('token')
        test_mode = webhook_data.get('test_mode', False)

        # Webhook replay protection (HIGH #8). CloudPayments delivers a
        # TransactionId per attempt; combined with event type ("pay") it gives
        # a stable per-delivery id. Note: external_id uniqueness still blocks
        # duplicate credits — this layer adds defence against forged event_id
        # replays in restore-missing-payment paths.
        from app.utils.webhook_replay import claim_webhook_event

        if transaction_id_cp:
            replay_event_id = f'pay:{transaction_id_cp}'
            if not await claim_webhook_event(db, 'cloudpayments', replay_event_id):
                logger.info(
                    'CloudPayments webhook повтор — игнорируем',
        ...
        if not payment:
            # HIGH #9: Refuse to create a brand-new local payment row from
            # webhook body unless the operator has explicitly opted in.
            # Acknowledge with 200 OK (caller returns True) so the provider
            # stops retrying, but DO NOT credit.
            if not getattr(settings, 'ALLOW_RESTORE_MISSING_PAYMENTS', False):
                logger.warning(
                    'CloudPayments webhook для неизвестного invoice — пропускаем '
                    '(ALLOW_RESTORE_MISSING_PAYMENTS=false)',
                    invoice_id=invoice_id,
                    amount_kopeks=amount_kopeks,
            ...
            # Cap restored payments to avoid leaked-key forgeries inflating
            # balances. Anything above the cap is logged for manual review.
            max_kopeks = getattr(settings, 'RESTORE_MISSING_PAYMENT_MAX_KOPEKS', 500_000)
            if amount_kopeks > max_kopeks:
                logger.error(
                    'CloudPayments restore exceeds RESTORE_MISSING_PAYMENT_MAX_KOPEKS — '
