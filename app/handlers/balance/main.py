...
async def handle_successful_topup_with_cart(user_id: int, amount_kopeks: int, bot, db: AsyncSession):
    ...
    # Cart-based subscription resume removed (was VPN-specific).
    # Kept as no-op stub so payment service callers don't error.
    try:
        _ = (storage, key, amount_kopeks)
    except Exception as e:
        logger.error('Ошибка обработки успешного пополнения', error=e)
