...
# Stubs for deleted VPN/subscription modules.  These functions/classes were
# removed when the VPN integration was deleted; kept here only so this
# module still imports cleanly.  Any handler that actually calls them will
# raise at runtime (which is fine — those code paths are dead).
class Subscription:  # type: ignore[no-redef]
    pass


class SubscriptionStatus:  # type: ignore[no-redef]
    class _V:
        value = ''
...
class RemnaWaveService:  # type: ignore[no-redef]
    pass


def resolve_hwid_device_limit_for_payload(*args, **kwargs):  # type: ignore[no-redef]
    return None


async def get_all_tariffs(*args, **kwargs):  # type: ignore[no-redef]
    return []


async def get_tariff_by_id(*args, **kwargs):  # type: ignore[no-redef]
    return None


async def get_all_server_squads(*args, **kwargs):  # type: ignore[no-redef]
    return [], 0


async def get_server_squad_by_id(*args, **kwargs):  # type: ignore[no-redef]
    return None


async def get_server_squad_by_uuid(*args, **kwargs):  # type: ignore[no-redef]
    return None
...
async def _render_user_subscription_overview(
    ...
    if settings.is_multi_tariff_enabled() and not subscription_id:
        pass  # stub: removed VPN/subscription import
...
async def get_detailed_referral_stats(db: AsyncSession, user_id: int) -> dict:
    ...
    for referral in referrals:
        ...
        if ref_sub:
            pass  # stub: removed deleted-model import
...
async def _add_subscription_traffic(
    ...
    try:
        pass  # deleted import: pass  # deleted import (app.database.crud.subscription)
        pass  # stub: removed VPN/subscription import
