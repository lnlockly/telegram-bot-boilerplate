...
class _StubTrafficScheduler:
    ...
    async def run_fast_check_now(self):
        return []

    class _StubService:
        async def get_snapshot_age_minutes(self):
            return 0.0

        def get_fast_check_threshold_gb(self):
            return 0

    service = _StubService()


traffic_monitoring_scheduler = _StubTrafficScheduler()


async def get_subscriptions_statistics(db):
    return {
...
async def traffic_check_callback(callback: CallbackQuery):
    ...
    try:
        ...
        traffic_monitoring_scheduler_v2 = traffic_monitoring_scheduler
