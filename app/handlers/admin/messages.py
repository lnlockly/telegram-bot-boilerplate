...
async def get_expiring_subscriptions(db, days):
    return []
...
async def get_target_users_count(db: AsyncSession, target: str) -> int:
    ...
    if target in {
        'active', 'trial', 'no', 'expiring', 'expiring_subscribers',
        'expired', 'expired_subscribers', 'active_zero', 'trial_zero', 'zero',
    } or target.startswith('tariff_'):
        return 0
...
async def get_target_users(db: AsyncSession, target: str) -> list:
    ...
    if target in {
        'active', 'trial', 'no', 'expiring', 'expired',
        'active_zero', 'trial_zero', 'zero',
        'expiring_subscribers', 'expired_subscribers',
        'canceled_subscribers', 'trial_ending', 'trial_expired',
        'autopay_failed',
    }:
        return []
