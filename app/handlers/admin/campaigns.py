...
async def get_all_server_squads(*args, **kwargs):
    return [], 0


async def get_server_squad_by_id(*args, **kwargs):
    return None


async def get_all_tariffs(*args, **kwargs):
    return []


async def get_tariff_by_id(*args, **kwargs):
    return None
...
async def _ensure_subscription_campaign(message_or_callback, campaign) -> bool:
    if isinstance(message_or_callback, types.CallbackQuery):
        await message_or_callback.answer(
            '❌ Подписки удалены из бота',
            show_alert=True,
        )
    else:
        await message_or_callback.answer('❌ Подписки удалены из бота')
    return False
...
async def process_edit_campaign_subscription_days(
    ...
    await update_campaign(db, campaign)
    await state.clear()
...
async def start_edit_campaign_tariff(
    ...
    await callback.answer('❌ Тарифы удалены из бота', show_alert=True)
    return
