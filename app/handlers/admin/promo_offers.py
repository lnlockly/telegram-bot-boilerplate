...
async def get_server_squad_by_uuid(*args, **kwargs):
    return None
...
def get_display_subscription_link(*args, **kwargs):
    return None
...
def _extract_offer_active_hours(offer, template: PromoOfferTemplate | None) -> int | None:
    ...
    if template and template.offer_type == 'test_access':
        return None
...
async def _handle_edit_field(
    ...
    try:
        if field == 'message_text':
            ...
            hours = max(1, int(value))
            await update_promo_offer_template(db, template)
