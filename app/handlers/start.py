...
from app.database.models import PinnedMessage, UserStatus
...
async def get_main_menu_text(user, texts, db: AsyncSession):
    ...
    try:
        base_text = texts.MAIN_MENU.format(user_name=user_name, subscription_status='')
...
async def get_main_menu_text_simple(user_name, texts, db: AsyncSession):
    ...
    try:
        base_text = texts.MAIN_MENU.format(user_name=safe_name, subscription_status='')
    except (KeyError, IndexError):
        base_text = texts.t(
            'MAIN_MENU_PLAIN',
            '👋 Здравствуйте, {user_name}!\n\nВыберите действие:',
        ).format(user_name=safe_name)
