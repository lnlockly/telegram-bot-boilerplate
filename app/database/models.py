...
class LandingPage(Base):
    ...
    display_order = Column(Integer, nullable=False, default=0)
    discount_percent = Column(Integer, nullable=True)  # 1-99, global discount
...
class ProcessedWebhookEvent(Base):
    """Webhook replay protection — records (provider, event_id) we've already
    processed. A captured-and-replayed webhook with the same event_id will hit
    the unique constraint and be rejected without crediting the user.

    Schema matches migration 37ebcf6eea38_webhook_events.py exactly.
    """
