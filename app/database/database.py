...
def _build_pg_connect_args(url: str) -> dict:
    """Build asyncpg connect_args, honouring DSN query params.

    asyncpg ignores ?sslmode=/?sslrootcert= from the DSN — they must be passed
    explicitly via connect_args. Pooled providers (Neon -pooler, Supabase
    pgbouncer, PgBouncer transaction-mode in general) also break prepared
    statements, so we disable the asyncpg statement cache when we detect them.
    """
    args: dict = {
        'server_settings': {
            'application_name': 'telegram-bot-boilerplate',
            'jit': 'on',
    ...
    if sslmode and sslmode != 'disable':
        # asyncpg accepts: True/False, 'prefer', 'require', 'verify-ca', 'verify-full', or SSLContext
        args['ssl'] = sslmode if sslmode in ('prefer', 'require', 'verify-ca', 'verify-full') else 'require'

    host = (parsed.hostname or '').lower()
    if '-pooler.' in host or 'pgbouncer' in host or qs.get('pgbouncer'):
        # PgBouncer transaction-mode is incompatible with asyncpg's prepared-statement cache.
...
_pg_connect_args = _build_pg_connect_args(DATABASE_URL) if not IS_SQLITE else {}
...
engine = create_async_engine(
    ...
    execution_options={} if IS_SQLITE else {'isolation_level': 'READ COMMITTED'},
    **pool_kwargs,
)
