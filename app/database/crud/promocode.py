...
async def increment_promocode_uses_atomic(db: AsyncSession, promocode_id: int) -> bool:
    """
    Atomically increment current_uses only if current_uses < max_uses.
    Returns True if the increment succeeded (slot reserved), False if exhausted.
    Caller is responsible for committing/rolling back the surrounding transaction.
    """
    result = await db.execute(
        update(PromoCode)
        .where(PromoCode.id == promocode_id, PromoCode.current_uses < PromoCode.max_uses)
