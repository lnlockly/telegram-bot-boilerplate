...
async def get_completed_transaction_by_external_id(
    db: AsyncSession,
    external_id: str,
    payment_method: PaymentMethod,
) -> Transaction | None:
    """Return a completed transaction matching external_id+payment_method, locked FOR UPDATE.

    Used by webhook processors to enforce idempotency: callers should short-circuit
    (acknowledge success) if a completed transaction already exists for the given
    provider charge id, so retried webhooks don't credit the balance twice.
    """
    result = await db.execute(
        select(Transaction)
        .where(
