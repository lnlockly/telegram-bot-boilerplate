...
async def get_user_by_remnawave_uuid(db: AsyncSession, remnawave_uuid: str) -> User | None:
    # VPN/Subscription modules deleted -- User no longer has remnawave_uuid.
    return None
...
async def get_users_list(
    ...
    # Subscription-level filters removed (Subscription model deleted)
    ...
    if order_by_traffic:
        # Subscription model deleted -- no traffic data; fallback to created_at
...
async def get_users_for_promo_segment(db: AsyncSession, segment: str) -> list[User]:
    ...
    # Subscription segments unsupported (Subscription model deleted)
    if segment == 'no_subscription':
        query = base_query
    else:
        logger.warning('Сегмент подписки больше не поддерживается', segment=segment)
        return []
...
async def get_inactive_users(db: AsyncSession, months: int = 3) -> list[User]:
    ...
    # Subscription model deleted -- no longer filter out users with recent subscriptions.
    result = await db.execute(
        select(User)
        .options(
            selectinload(User.user_promo_groups).selectinload(UserPromoGroup.promo_group),
...
async def get_users_with_active_subscriptions(db: AsyncSession) -> list[User]:
    """Subscription model deleted -- always returns empty list."""
    return []
...
async def lock_user_subscriptions_for_update(db: AsyncSession, user_id: int) -> list:
    """Subscription model deleted -- returns empty list."""
    return []
