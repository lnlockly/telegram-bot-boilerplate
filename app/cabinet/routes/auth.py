...
async def _sync_subscription_from_panel_by_email(db: AsyncSession, user: User) -> None:
    ...
    The original implementation imported a user's existing RemnaWave VPN
    subscription based on their verified email. The VPN stack was removed
    during the boilerplate conversion, so this helper now intentionally
    does nothing and is preserved only to keep its call sites compiling.
    """
    return None
